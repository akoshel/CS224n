...
def minibatch_parse(sentences, model, batch_size):
    ...
    for batch in range(int(math.ceil(len(sentences)/batch_size))):
        ...
        partial_parses = [PartialParse(s) for s in sentences[start_idx: start_idx + batch_size]]
        unfin_partialParses = partial_parses.copy()
        while len(unfin_partialParses) > 0:
            transitions = model.predict(unfin_partialParses)
            for idx, tr in enumerate(transitions):
                unfin_partialParses[idx].parse([tr])
