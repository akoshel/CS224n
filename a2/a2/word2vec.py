...
from utils.gradcheck import grad_tests_negsamp, grad_tests_softmax, gradcheck_naive
...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    ### END YOUR CODE
    # print(centerWordVec.shape, outsideVectors.shape, outsideWordIdx)
    y = softmax(centerWordVec.dot(outsideVectors.T))
    loss = -np.log(y[outsideWordIdx])
    y[outsideWordIdx] -= 1
    gradCenterVec = y.dot(outsideVectors)
    gradOutsideVecs = y.reshape(-1, 1).dot(centerWordVec.reshape(1, -1))
...
def negSamplingLossAndGradient(
    ...
    gradCenterVec = np.zeros(shape=centerWordVec.shape)
    gradOutsideVecs = np.zeros(shape=outsideVectors.shape)
    y = sigmoid(centerWordVec.dot(outsideVectors.T))
    loss = -np.log(y[outsideWordIdx])
    for k in negSampleWordIndices:
        loss += -np.log(sigmoid(-centerWordVec.dot(outsideVectors[k].T)))
    gradCenterVec = (
        -(1 - sigmoid(centerWordVec.dot(outsideVectors[outsideWordIdx].T)))
        * outsideVectors[outsideWordIdx]
    )
    for k in negSampleWordIndices:
        gradCenterVec += (
            1 - sigmoid(-centerWordVec.dot(outsideVectors[k].T))
        ) * outsideVectors[k]
    ...
    for k in negSampleWordIndices:
        gradOutsideVecs[k] += (
            1 - sigmoid(-centerWordVec.dot(outsideVectors[k]))
        ) * centerWordVec
...
def skipgram(
    ...
    for outsideWord in outsideWords:
        loss1, gradCenterVec1, gradOutsideVectors1 = word2vecLossAndGradient(
